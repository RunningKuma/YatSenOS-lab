...
fn main() -> isize {
    ...
    for i in 0..16 {
        ...
        if pid == 0 {
            if i % 2 == 0 {
                unsafe {
                    PIDS_PRODUCERS[i / 2] = sys_get_pid();
                }
                producer();
            } else {
                unsafe {
                    PIDS_CONSUMERS[i / 2] = sys_get_pid();
                }
                consumer();
            }
        }
    ...
    for i in 0..8 {
        println!("Waiting for Producer PID[{}]: {}", i, unsafe {
            PIDS_PRODUCERS[i]
        });
        sys_wait_pid(unsafe { PIDS_PRODUCERS[i] });
        println!("Waiting for Consumer PID[{}]: {}", i, unsafe {
            PIDS_CONSUMERS[i]
        });
...
fn producer() -> ! {
    ...
    for _ in 0..10 {
        ...
        println!(
            "Producer {} produced message, total count: {}",
            pid,
            unsafe { COUNT }
        );
...
fn consumer() -> ! {
    ...
    for _ in 0..10 {
        ...
        println!(
            "Consumer {} consumed message, total count: {}",
            pid,
            unsafe { COUNT }
        );
